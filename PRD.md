...
> **版本**: v1.1 | **日期**: 2026-07-26 | **状态**: 方向收敛：动作展示库 + 健身训练记录
...
| 层面 | 选型 | 说明 |
|------|------|------|
| 前端 | React + Vite | 静态 SPA，开发和部署简单 |
| UI | Tailwind CSS + shadcn/ui | 移动端优先，组件可控 |
| 本地数据库 | IndexedDB | 保存训练记录、收藏、最近动作 |
| 状态管理 | React Query / Zustand | 轻量管理列表、表单和本地数据状态 |
| 搜索 | 前端本地索引 | 1,324 条动作数据可直接在浏览器搜索 |
| PWA | vite-plugin-pwa | 后续增强离线体验 |
| 部署 | Vercel / 静态托管 | MVP 无需后端服务 |

### 3.2 项目目录

项目采用企业级前端 monorepo 结构，详见 `docs/project-structure.md`。MVP 阶段优先创建 `apps/web`、`packages/domain`、`packages/exercise-data`、`packages/local-db` 和 `data/dictionaries`，暂不创建后端服务。
...
| ADR-002 | MVP 暂不引入后端和 AI | 聚焦动作库与记录闭环，降低复杂度 |
